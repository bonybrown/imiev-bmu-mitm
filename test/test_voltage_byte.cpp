...
TEST_GROUP(VoltageByte_SoC2Calculation)
{
    void setup() {
    }
...
TEST(VoltageByte_SoC2Calculation, KnownVoltageTable)
{
    ...
    for (size_t i = 0; i < sizeof(testCases) / sizeof(TestCase); i++) {
        ...
        float actualSoC = vb.toSoC2();
        DOUBLES_EQUAL(tc.expectedSoC, actualSoC, tc.tolerance);
    }
}

TEST(VoltageByte_SoC2Calculation, Monotonicity)
{
...
TEST(VoltageByte_SoC2Calculation, BoundaryClamping)
{
