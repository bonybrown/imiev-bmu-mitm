...
#define CATL93_CAPACITY 90.0 // Battery Capacity in Ah
...
void can_imiev_handler(uint8_t can_bus, CAN_FRAME *frame) // FSI handler for Imiev only
{
	...
	static uint8_t valid_data_counter = 0;	// Counter for valid data
	static uint8_t initialization_flag = 0; // power up flag
	...
	// and powerup , first voltage is initialized to the lowest possible
	static VoltageByte vMin = VoltageByte::fromVoltage(2.76f); // it should be refreshed to a better value immediatly afterward.

	switch (frame->ID)
	{
	case 0x373:
	{
		// Parse battery data message using type-safe class
		CanMessage373 batteryData(frame);

		// Extract cell minimum voltage
		VoltageByte cellMinVoltage = batteryData.getCellMinVoltage();
		...
		{ // YES!  we gonna initialize some few things
			if (valid_data_counter++ >= 20)
			{											   // wait 20 0x373 frames until CMU is providing good data
				SoC2 = vMin.toSoC2();					   // get battery SoC based on voltage (without current!)
				remAh1 = (SoC2 * CATL93_CAPACITY) / 100.0; // calculate the initial remaining Ah in the battery based on voltage
				remAh2 = remAh1;						   // calculate the start remaining Ah in the battery based on voltage
				initialization_flag = 1;
			}
		}

		float pack_amps = batteryData.getPackCurrent();
		float Ah = pack_amps / 360000.0; // Amphours to or from the battery in the 0.01 sec between 0x373 frames
		...
		if (pack_amps > -CURRENT_FOR_SOC_VOLTAGE_EVAL && pack_amps < CURRENT_FOR_SOC_VOLTAGE_EVAL)
	...
	{ // 100ms reccurence
		...
		CanMessage374 socData(frame);
		socData.setBatteryCapacity(2 * CATL93_CAPACITY); // modify to 2 x the capacity
		...
		if (SoC1 < 0)
		{
			SoC1 = 0; // clamp SoC1 value for real life
		}
		...
		{											   // when battery current has been low for long enough  for battery voltage to settle
			SoC2 = vMin.toSoC2();					   // gets SoC based on lowest cell voltage
			remAh2 = (SoC2 * CATL93_CAPACITY) / 100.0; // correct remaining capacity based on voltage
		}
		else
		{											   // current has not been low, long enough to use the SoC based on voltage
			...
			if (SoC2 < 0)
			{
				SoC2 = 0; // clamp SoC2 value for real life
			}
		}
		// modify data coming from BMU
		socData.setSoC1(SoC1);
		socData.setSoC2(SoC2);
		break;
	}
	}
...
void StateMachine() // state machine management
{
	...
	case NoRegen:
		if (uwTick - OldEventTime1ms >= NextEventDelay1ms)
		{
			...
			// action
			LedState = LedBlinkOnce; //
									 // if (TestMinTemp(MIN_TEMP + 2))
									 // {							  // take car of hysteresis
...
void LedStateMachineUnderSampled(uint8_t LedOrder)
{
	...
	else if ((TickUnderSAmple) && !(uwTick & (uint32_t)128)) // H=>L edge detection
	{
		LedStateMachine(LedOrder);
		TickUnderSAmple = 0;
	}
...
void LedStateMachine(uint8_t LedOrder) // LED state machine management called ~128ms by LedStateMachineUnderSampled
{
	...
	if (LedOrder == LED_FORCE_OFF)
		LedState = LedOffState;
